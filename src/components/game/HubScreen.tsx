  ...
  return (
    <div className="flex h-full min-h-0 flex-col overflow-y-auto p-3">
      <PixelWindow className="mb-3 rounded-none">
        ...
        <p className="mt-1 text-xs text-muted">魂に刻んだ遺物と、所持カード・ルーン。</p>
      </PixelWindow>
      ...
      <p className="mb-2 text-xs tracking-widest text-muted">カード {inventory.cards.length}</p>
      <div className="mb-4 grid grid-cols-3 gap-2 sm:grid-cols-4 md:grid-cols-5">
        {inventory.cards.map((card) => (
          <CollectionCard key={card.instanceId} instance={card} />
        ))}
      </div>
